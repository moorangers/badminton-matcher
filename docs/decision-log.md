...
## ADR-019 — "เปลี่ยน PIN" (ไม่ทำ "ลืม PIN" เต็มรูปแบบ) + เพิ่ม loading state ทั่วแอปหลังย้ายไป MongoDB

- **วันที่:** 2026-09-07
- **สถานะ:** decided
- **Context:** ผู้ใช้ถามว่าคนสร้าง session ลืม PIN แล้วทำไงได้บ้าง — แอปนี้ไม่มีระบบ account/email เลย (auth ต่อ session ด้วย PIN อย่างเดียว ตาม ADR-007) ทางแก้เดียวตอนนี้คือสร้าง session ใหม่ (เสียเวลาแอดผู้เล่นใหม่หมด) หรือไปแก้ `adminPinHash` ตรงๆ ใน DB วิเคราะห์ร่วมกันแล้วสรุปว่า flow "ลืม PIN" เต็มรูปแบบ (ต้องมีการยืนยันตัวตนแบบอื่นที่ไม่ใช่ PIN) ไม่คุ้มที่จะทำตอนนี้ เพราะเคสที่เจอจริงส่วนใหญ่คือ "ยังมีเครื่องที่ login ค้างอยู่ แค่อยากเปลี่ยน PIN ใหม่" ไม่ใช่ "ทุกเครื่อง logout หมดแล้ว" — ผู้ใช้เลยเสนอว่าทำแค่ "เปลี่ยน PIN" (ต้องอยู่ในหน้า dashboard ที่ authenticated แล้วเท่านั้น) พอสำหรับตอนนี้ ข้อมูล session อื่นไม่กระทบ แค่ update PIN ใหม่
  - ผู้ใช้ยังตั้งข้อสังเกตแยกว่าตอนนี้ต่อ MongoDB จริงแล้ว (ไม่ใช่ localStorage เหมือนก่อน) ปุ่มต่างๆ ที่เคยตอบสนองทันทีอาจมี latency จาก network round-trip ที่มองไม่เห็น ขอให้เพิ่ม loading indicator (spinner/skeleton) ให้ปุ่มที่ยิง API ทั่วแอป ไม่ใช่แค่ฟีเจอร์ใหม่นี้
- **Decision:** (1) เพิ่ม `POST /api/sessions/:id/change-pin` รับ `currentPin`+`newPin` ตรวจ `currentPin` ตรงกับ `adminPinHash` เดิมก่อน (ผ่าน `verifyPin`) ถึงจะ hash `newPin` ใหม่บันทึกทับ — เป็นจุดเดียวในแอปที่ตรวจ auth ฝั่ง server จริงจัง (ที่อื่นยังเป็น client-side PIN gate ล้วนตาม ADR-007) เพราะเป็น action ที่กระทบ credential โดยตรง; (2) ปุ่ม "เปลี่ยน PIN" อยู่ในการ์ด QR/ลิงก์เช็คอิน (การ์ดที่มีลิงก์ให้แชร์ตอนเปิด session) เปิด modal ให้กรอก PIN ปัจจุบัน + PIN ใหม่ (4-6 หลัก); (3) เพิ่ม loading state (`isXLoading` boolean หรือ `xId: string | null` สำหรับ action ต่อรายการ) ให้ปุ่มที่ยิง API หลักๆ ทั่วแอป — เพิ่ม/ลบ/เช็คอินผู้เล่น, บันทึกชื่อผู้เล่น, สลับผู้เล่น, บันทึกแผนรอบถัดไป, เริ่มจับคู่, รีเซ็ตสถิติ, เริ่ม/จบแมตช์/ปิดคอร์ด/ย้อนกลับต่อคอร์ด (ใช้ `pendingCourt: number | null` ตัวเดียวคุมทุกปุ่มในการ์ดคอร์ดนั้น ปุ่มคอร์ดอื่นไม่ถูกล็อกตาม) — ทุกจุดใช้ pattern เดียวกัน: ปิดปุ่ม (`disabled`) + เปลี่ยนไอคอนเป็น `Loader2` หมุนระหว่างรอ API ตอบ
- **Consequences:** ปุ่ม "ปิดคอร์ด" ที่หัวการ์ด (เปิด confirm modal) ไม่โชว์ spinner ของตัวเอง เพราะ modal ปิดทันทีที่กดยืนยันแล้ว async work ไปเกิดหลังจากนั้น (การ์ดคอร์ดจะโชว์ spinner แทนผ่าน `pendingCourt` ที่ยังเห็นอยู่หลัง modal ปิด) — ยังไม่มี "ลืม PIN" แบบเต็ม ถ้าทุกเครื่อง logout หมดจริงๆ ทางแก้เดียวยังคงเป็นสร้าง session ใหม่หรือแก้ DB ตรงๆ (ตั้งใจเก็บไว้พิจารณาอีกทีถ้าเกิดเคสจริง) — **แก้เพิ่มทันทีใน ADR-020** เพราะพบว่า "เปลี่ยน PIN" เพียงอย่างเดียวแก้ปัญหาลืม PIN จริงไม่ได้ (ต้องเข้า dashboard ให้ได้ก่อนถึงจะเปลี่ยนได้ ซึ่งถ้าลืม PIN ก็เข้าไม่ได้ตั้งแต่แรก)

## ADR-020 — ลิงก์เข้าใช้งานส่วนตัว (แนบ PIN ในลิงก์) แทนที่จะพึ่งอีเมล/ระบบ account

- **วันที่:** 2026-09-07
- **สถานะ:** decided
- **Context:** ผู้ใช้ทักท้วง ADR-019 ว่า "เปลี่ยน PIN" ที่ต้องเข้า dashboard (authenticated) ก่อนถึงจะใช้ได้ ไม่ช่วยเคส "ลืม PIN" จริงเลย เพราะถ้าลืมก็เข้า dashboard ไม่ได้ตั้งแต่แรก วนกลับไปเป็นปัญหาเดิม — ผู้ใช้เสนอให้แนบ PIN ไปกับ "ลิงก์ที่ copy/share" เพื่อให้เปิดลิงก์แล้ว auto กรอก PIN ให้เลย แต่การ์ดลิงก์ที่มีอยู่ (`checkinUrl` ในการ์ด "ลิงก์ลงชื่อ/เช็คอินด้วยตัวเอง") เป็นลิงก์สาธารณะไม่ต้องใช้ PIN ที่ตั้งใจแชร์ให้ผู้เล่นทุกคน — ถ้าแนบ PIN ไปกับลิงก์นั้นจะเท่ากับรั่ว PIN แอดมินให้ผู้เล่นทุกคนที่ได้ลิงก์ ชี้ประเด็นนี้กับผู้ใช้แล้วถามกลับ ผู้ใช้ยืนยันว่าต้องการ "ลิงก์ส่วนตัวอันใหม่ แยกจากลิงก์เช็คอิน" ตามที่แนะนำ — แยกกันถามอีกครั้งเรื่องส่งอีเมลให้ผู้สร้าง session (เป็นอีกทางแก้ปัญหาเดียวกัน) แต่ผู้ใช้ตัดสินใจว่าไม่คุ้มเพราะต้องต่อ email provider ใหม่ + API key + field อีเมล ขอให้ทำแค่ "หลังสร้าง session เสร็จ โชว์ลิงก์พร้อม PIN ให้ copy ไว้เลย" แทน ไม่ต้องมี email
- **Decision:** (1) เพิ่ม query param `?sid=<sessionId>&pin=<pin>` ที่หน้า root (`/`) — bootstrap effect เช็คก่อน localStorage เสมอ ถ้ามีทั้งคู่จะ fetch session + เรียก `verifySessionPin` อัตโนมัติ สำเร็จก็ตั้ง `isAuthenticated=true` ตรงเข้า dashboard เลย (ถ้า PIN ผิด/เปลี่ยนไปแล้วก็ fallback ไปหน้า PIN gate ปกติเงียบๆ ไม่โชว์ error) แล้ว `history.replaceState` ตัด query ออกจาก URL ทันทีไม่ให้ค้างอยู่ในแอดเดรสบาร์; (2) เพิ่มปุ่ม "ลิงก์ส่วนตัว" (ไอคอน `Link2`) คู่กับ "เปลี่ยน PIN" ในหัวการ์ด "ลิงก์ลงชื่อ/เช็คอินด้วยตัวเอง" — เป็นลิงก์คนละอันกับ `checkinUrl` โดยเจตนา เปิด modal ให้ยืนยัน PIN ปัจจุบันก่อน (เรียก `verifySessionPin` เพื่อ confirm ถูกต้องก่อนฝัง PIN ลง URL) ถึงจะสร้างลิงก์ + โชว์ปุ่มคัดลอก/แชร์ พร้อมข้อความเตือนชัดเจนว่า "มีค่าเหมือนรู้ PIN ห้ามแชร์ให้ผู้เล่น"; (3) **auto-generate ลิงก์นี้ทันทีหลังสร้าง session สำเร็จ** (ใช้ PIN ที่กรอกตอนสร้างอยู่แล้ว ไม่ต้องให้พิมพ์ซ้ำ) เปิด modal เดียวกันขึ้นมาเลยพร้อมลิงก์ที่พร้อมคัดลอก — แก้ปัญหาต้นตอ "สร้างเสร็จปิดแท็บทิ้งไม่เคยบันทึกอะไรไว้เลย" โดยไม่ต้องมีอีเมล/account ใดๆ
- **Consequences:** ต้องแก้ `tests/e2e/helpers.ts`'s `createSession()` ให้ปิด modal ลิงก์ส่วนตัวที่เด้งขึ้นอัตโนมัติหลังสร้าง session ก่อนถึงจะกดต่อได้ (มันบัง modal ทับปุ่มเลือกโหมด) — ไม่ทำระบบอีเมลตามที่ถามถึง เพราะผู้ใช้ตัดสินใจว่าลิงก์พร้อม copy ตอนสร้างเสร็จก็พอแล้ว ไม่คุ้มกับ infra ที่ต้องเพิ่ม (provider, API key, field อีเมล) — ยังมีความเสี่ยงเดิมที่ต้องยอมรับ: ใครก็ตามที่ได้ลิงก์นี้ (เช่น หลุดจาก screenshot, chat log, browser history ที่แชร์เครื่อง) จะเข้าถึง dashboard ได้ทันทีเหมือนรู้ PIN — เป็น trade-off ที่ยอมรับได้เพราะสอดคล้องกับโมเดล security เดิมของแอป (PIN เดียวต่อ session, ไม่มี account, ADR-006/007) อยู่แล้ว
